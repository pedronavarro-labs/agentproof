...
- **Experimental runtime pilot:** an in-process dispatch gate and synthetic read/write example are available for feedback. Next: a genuine MCP transport adapter, verified identity and scopes, durable audit records, and trusted human approval. No production enforcement claim.
- **MCP stdio pilot:** a narrow local server handles `tools/call` with a durable SQLite decision journal. Next: SDK interoperability and a protected Streamable HTTP deployment with OAuth, independently verified scopes, separate human approval, externally anchored audit, and bypass testing.
