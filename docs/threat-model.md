...
The experimental in-process gate protects only tool invocations passed through its `call` method. It trusts host-provided bindings and scopes, denies all writes, and stores an in-memory metadata-only audit. Direct handler access, alternate connections, forged host context and altered manifests remain outside its protection. See [runtime pilot](runtime-pilot.md).
